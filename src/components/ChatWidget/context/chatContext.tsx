...
export const ChatProvider = ({ children, demoConfig, currentView, isDemo = false }: ChatProviderProps): JSX.Element => {
    ...
    const handleSetFormData = useCallback(async (formData: Record<string, string>) => {
        ...
        try {
            const response = await fetch("https://dev-socket.pullseai.comapi/widgets/createContactDevice/" + apiKey, {
                method: 'POST',
                headers: {
    ...
    const fetchWidgetConfig = useCallback(async () => {
        try {
            ...
            const url = `https://dev-socket.pullseai.comapi/widgets/getWidgetConfig/${apiKey}?workspace_id=${encodeURIComponent(workspaceId)}`;
