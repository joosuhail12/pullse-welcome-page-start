  ...
  try {
    ...
    return await withResilience(
      async () => {
        ...
        const url = `https://dev-socket.pullseai.comapi/widgets/getWidgetConfig/${apiKey}?workspace_id=${encodeURIComponent(sanitizedWorkspaceId)}`;
        ...
        if (config.sessionId && !chatSessionId) {
        ...
        // Add realtime flag to support existing code
...
export const fetchConversations = async () => {
  ...
  const response = await fetch(`https://dev-socket.pullseai.comapi/widgets/getContactDeviceTickets`, {
    headers: {
      'Content-Type': 'application/json',
...
export const fetchConversationByTicketId = async (ticketId: string) => {
  try {
    ...
    const response = await fetch(`https://dev-socket.pullseai.comapi/widgets/getConversationWithTicketId/${ticketId}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${accessToken}`
