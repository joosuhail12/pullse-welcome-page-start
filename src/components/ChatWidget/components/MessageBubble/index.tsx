...
const MessageBubble: React.FC<MessageBubbleProps> = ({
  ...
  return (
    <div className={cn(
      "group relative flex gap-4 px-6 py-5 transition-all duration-500",
      isUser ? "justify-end" : "justify-start",
      "hover:bg-gradient-to-r hover:from-transparent hover:via-gray-50/60 hover:to-transparent",
      "animate-subtle-fade-in"
    )}>
      {/* Agent Avatar - Left side */}
      {isAgent && (
        <div className="flex-shrink-0 relative transform transition-transform duration-300 group-hover:scale-105">
          <MessageAvatar
            src={agentAvatar}
            alt="Agent"
            isAgent={true}
      ...
      <div className={cn(
        ...
        <div
          className={cn(
            "relative px-5 py-4 rounded-3xl transition-all duration-500 group-hover:shadow-xl",
            "backdrop-blur-sm border transform group-hover:scale-[1.01]",
            isUser ? [
              "ml-auto rounded-br-md",
              "bg-gradient-to-br from-blue-500 via-blue-600 to-indigo-700",
              "text-white border-blue-300/30",
              "shadow-lg shadow-blue-500/30",
              "hover:shadow-2xl hover:shadow-blue-500/40",
              "before:absolute before:inset-0 before:rounded-3xl before:rounded-br-md",
              "before:bg-gradient-to-br before:from-white/10 before:to-transparent before:opacity-0 hover:before:opacity-100 before:transition-opacity before:duration-300"
            ] : [
              "mr-auto rounded-bl-md",
              "bg-white/95 border-gray-200/60",
              "text-gray-800 shadow-lg shadow-gray-500/15",
              "hover:bg-white hover:shadow-xl hover:shadow-gray-500/25",
        ...
        >
          ...
          <div className={cn(
            ...
            isUser ? [
              ...
              "-left-1 bottom-4",
              "bg-white/95 border-l border-b border-gray-200/60",
              "transform rotate-45",
              "shadow-md shadow-gray-500/15"
            ]
          )} />
        </div>
        ...
        {message.reactions && message.reactions.length > 0 && (
          <div className={cn(
            ...
            {message.reactions.map((reaction, index) => (
              <span
                key={index}
                className="inline-flex items-center px-3 py-1.5 rounded-full bg-gray-100/95 hover:bg-gray-200/95 text-xs font-medium transition-all duration-300 cursor-pointer transform hover:scale-110 border border-gray-200/60 shadow-sm hover:shadow-md"
              >
                {reaction}
              </span>
            ))}
